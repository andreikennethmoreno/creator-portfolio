          ...
          <div className="p-6 space-y-3">
            {CONFIG.dev.education.map((edu, i) => (
              <div key={i} className="flex items-start justify-between gap-3">
                <div className="flex gap-3 min-w-0">
                  <GraduationCap className="size-5 text-foreground shrink-0 mt-0.5 opacity-60" />
                  <div>
                    <p className="text-sm font-semibold text-foreground">{edu.school}</p>
                    <p className="text-xs text-foreground">{edu.degree}</p>
                  </div>
                </div>
                <span className="text-xs text-foreground font-mono shrink-0 whitespace-nowrap">
                  {edu.period}
                </span>
              </div>
            ))}
            ...
            <div className="space-y-3">
              ...
              {CONFIG.dev.certificates.map((cert, i) => (
                <div key={i} className="flex items-start justify-between gap-3">
                  <div className="flex gap-3 min-w-0">
                    <Award className="size-5 text-foreground shrink-0 mt-0.5 opacity-60" />
                    <div>
                      <p className="text-sm text-foreground">{cert.title}</p>
                      <p className="text-xs text-foreground">{cert.issuer}</p>
                    </div>
                  </div>
                  <span className="text-xs text-foreground font-mono shrink-0 whitespace-nowrap">
                    {cert.year}
