...
- **Three dock sections** (visible in desktop mode):
  ...
  - **Center dock**: Search button (in desktop mode), social links (YouTube, Threads, GitHub, Email), app launchers for each section, wallpaper cycler, card style toggle
...
| `/api/refresh-threads-token` | `src/app/api/refresh-threads-token/route.ts` | GET — refreshes Threads long-lived access token |
...
| **Vercel API** | Fetch top projects with deployment counts | `VERCEL_API_TOKEN`, `VERCEL_TEAM_ID` | `revalidate: 3600` (1 hour) |
...
|---|---|---|---|
| `THREADS_ACCESS_TOKEN` | Yes | Meta Threads Graph API | Threads feed fetch + token refresh |
| `THREADS_USER_ID` | Yes | Meta Threads Graph API | Threads feed fetch |
...
| `VERCEL_API_TOKEN` | Yes | Vercel API | Top projects fetch |
| `VERCEL_TEAM_ID` | Yes | Vercel API | Top projects fetch |
